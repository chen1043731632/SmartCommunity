...
public class SmartOCActivity extends GosControlModuleBaseActivity {
    ...
    private void initevent(){
    	smart_oc_listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// TODO Auto-generated method stub
				AlertDialog.Builder builder = new AlertDialog.Builder(SmartOCActivity.this);
				...
				builder.setItems(items, new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						case 0:
							...
							Toast.makeText(getApplicationContext(), "删除完毕", Toast.LENGTH_SHORT).show();
							initdata();
							initList();
							break;
						case 1:
							if (!checkNetwork(SmartOCActivity.this)) {
								Toast.makeText(SmartOCActivity.this, R.string.network_error, Toast.LENGTH_SHORT).show();
							} else {
							Intent	intent = new Intent(SmartOCActivity.this, UpdataActivity.class);
    ...
    private void initdata(){
		...
		//Toast.makeText(getApplicationContext(), giz.get(0).getName(), Toast.LENGTH_SHORT).show();
	}
	Gizinfo gizinfo;
...
//				if(((Switch) view).isChecked()){
					try {
						if(((Switch) view).isChecked())
						sendJson(KEY_Sendcom, Integer.parseInt(giz.get(position).getAddress()));
						else
						sendJson(KEY_Sendcom, Integer.parseInt(giz.get(position).getAddress())+1);
						Toast.makeText(getApplicationContext(), "已发送指令...", Toast.LENGTH_SHORT).show();
...
//					}).setNegativeButton(R.string.no, null).show();
		...
		case R.id.action_addDevice:
			if (!checkNetwork(SmartOCActivity.this)) {
				...
				intent = new Intent(SmartOCActivity.this, AddOcdeviceActivity.class);
				intent.putExtra("bindgiz", MacAddress);
				startActivity(intent);
			}
			break;
		case R.id.action_site:
			intent = new Intent(SmartOCActivity.this, GosSettiingsActivity.class);
			startActivity(intent);
			break;
		}
