...
public class UpdataActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initdata();
		initevent ();
	//	Toast.makeText(getApplicationContext(), bindgiz, Toast.LENGTH_SHORT).show();
	...
	private void initevent (){
		bt_updatadevice.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(id!=0){
				Gizinfo gizinfo= new Gizinfo(id,ed_name.getText().toString(),ed_address.getText().toString(),bindgiz,"NULL",0);
				dbAdapter.update(gizinfo);
				Toast.makeText(getApplicationContext(), "修改完毕", Toast.LENGTH_SHORT).show();
				}else{
					Toast.makeText(getApplicationContext(), "修改对象错误", Toast.LENGTH_SHORT).show();
				}
			}
		});
	}
